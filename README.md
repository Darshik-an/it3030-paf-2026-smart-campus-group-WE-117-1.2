...
Demo Flow (Viva)
Start Docker MySQL + Backend + Frontend
Register a user via the signup page
Login → see dashboard with sidebar navigation
Open Postman → POST /api/notifications/test/seed with JWT in Authorization header
Refresh browser → notification bell shows 5 unread notifications
Click bell → dropdown shows notifications with mark-as-read functionality
(Optional) Google OAuth login if credentials are configured
Color Palette
Color	Hex	Usage
